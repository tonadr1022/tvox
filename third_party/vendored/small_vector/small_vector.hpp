...
concept LegacyContiguousIterator =
    std::random_access_iterator<I> && std::is_lvalue_reference<std::iter_reference_t<I>>::value &&
...
concept AllocatorFor =
    CopyConstructible<A> &&
    requires(A a, typename std::allocator_traits<A>::template rebind_alloc<U> b, A a1, A a2, U xp,
      ...
      requires std::same_as<typename A::value_type, typename std::allocator_traits<A>::value_type>;
...
class GCH_EMPTY_BASE allocator_inliner<Allocator, true> : private Allocator {
 public:
  ...
  constexpr explicit allocator_inliner(const Allocator& alloc) noexcept : Allocator(alloc) {}
...
class GCH_EMPTY_BASE allocator_interface : public allocator_inliner<Allocator> {
 ...
 private:
  ...
  struct is_trivially_constructible_impl<typename std::enable_if<is_complete<V>::value>::type, V,
  ...
  struct has_ptr_traits_to_address<
      P, void_t<decltype(std::pointer_traits<P>::to_address(std::declval<P>()))>> : std::true_type {
  };
  ...
  template <typename A, typename V, typename... Args>
  struct has_alloc_construct_impl<typename std::enable_if<is_complete<V>::value>::type, A, V,
  ...
  struct has_alloc_destroy_impl<void_t<decltype(std::declval<A&>().destroy(std::declval<V*>()))>, A,
                                V> : std::true_type {};
 ...
 public:
  ...
  constexpr explicit allocator_interface(const alloc_ty& alloc) noexcept : alloc_base(alloc) {}
  ...
  GCH_CPP20_CONSTEXPR void maybe_copy(const allocator_interface&) noexcept {}

  template <bool Propagate = alloc_traits::propagate_on_container_copy_assignment::value,
            typename std::enable_if<Propagate, bool>::type = true>
  GCH_CPP20_CONSTEXPR void maybe_copy(const allocator_interface& other) noexcept(
      std::is_nothrow_copy_assignable<alloc_base>::value) {
    alloc_base::operator=(other);
  }

  template <bool Propagate = alloc_traits::propagate_on_container_move_assignment::value,
            typename std::enable_if<!Propagate, bool>::type = false>
  GCH_CPP20_CONSTEXPR void maybe_move(allocator_interface&&) noexcept {}

  template <bool Propagate = alloc_traits::propagate_on_container_move_assignment::value,
            typename std::enable_if<Propagate, bool>::type = true>
  GCH_CPP20_CONSTEXPR void maybe_move(allocator_interface&& other) noexcept(
      std::is_nothrow_move_assignable<alloc_base>::value) {
    alloc_base::operator=(std::move(other));
  }

  template <bool Propagate = alloc_traits::propagate_on_container_swap::value,
            typename std::enable_if<!Propagate, bool>::type = false>
  GCH_CPP20_CONSTEXPR void maybe_swap(allocator_interface&) {}

  template <bool Propagate = alloc_traits::propagate_on_container_swap::value,
            typename std::enable_if<Propagate, bool>::type = true>
  GCH_CPP20_CONSTEXPR void maybe_swap(allocator_interface& other) {
    using std::swap;
  ...
  struct is_memcpyable<QualifiedFrom, QualifiedTo,
    ...
    static_assert(!std::is_reference<QualifiedTo>::value, "QualifiedTo must not be a reference.");
  ...
  struct is_contiguous_iterator
      : bool_constant<
            ...
            std::is_same<InputIt, decltype(std::begin(
  ...
  GCH_NODISCARD
  static GCH_CPP17_CONSTEXPR size_ty internal_range_length(std::move_iterator<ptr> first,
                                                           std::move_iterator<ptr> last) noexcept {
    // This is guaranteed to be less than or equal to max size_ty.
  ...
  GCH_CPP20_CONSTEXPR void_t<decltype(::new (std::declval<void*>()) V(std::declval<Args>()...))>
  construct(ptr p, Args&&... args) noexcept(noexcept(::new (std::declval<void*>())
                                                         value_ty(std::declval<Args>()...))) {
  ...
  GCH_CPP20_CONSTEXPR
  ptr uninitialized_fill(ptr first, ptr last) { return uninitialized_value_construct(first, last); }
...
class small_vector_data : public small_vector_data_base<Pointer, SizeT> {
 public:
  ...
  GCH_CPP14_CONSTEXPR
  T* storage(void) noexcept { return m_storage.get_inline_ptr(); }
...
class GCH_EMPTY_BASE
    small_vector_data<Pointer, SizeT, T, 0> : public small_vector_data_base<Pointer, SizeT> {
 public:
  small_vector_data(void) = default;
  small_vector_data(const small_vector_data&) = delete;
  small_vector_data(small_vector_data&&) noexcept = delete;
  ...
  GCH_CPP14_CONSTEXPR
  T* storage(void) noexcept { return nullptr; }
};
...
class small_vector_base : public allocator_interface<Allocator> {
 ...
 protected:
  ...
  struct is_emplace_constructible_impl<
      void_t<typename std::enable_if<is_complete<V>::value>::type,
             decltype(std::declval<AI&>().construct(std::declval<V*>(), std::declval<Args>()...))>,
  ...
  template <typename... Args>
  struct is_nothrow_emplace_insertable : is_emplace_insertable<Args...>::nothrow {};
  ...
  template <typename A>
  struct allocators_always_equal : bool_constant<std::is_same<std::allocator<value_ty>, A>::value
...
#else
    std::fprintf(stderr, "[gch::small_vector] The required allocation exceeds the maximum size.\n");
    std::abort();
#endif
  }
 ...
 protected:
  ...
  calculate_new_capacity(const size_ty current, const size_ty required) const noexcept {
    ...
    if (get_max_size() - current <= current) return get_max_size();
    ...
    if (new_capacity < required) return required;
    return new_capacity;
  }
  ...
  template <typename ForwardIt>
  GCH_CPP20_CONSTEXPR void overwrite_existing_elements(const ForwardIt first, const ForwardIt last,
                                                       const size_ty count) {
  ...
  template <typename ForwardIt,
            typename std::enable_if<is_nothrow_emplace_insertable<
                typename std::iterator_traits<ForwardIt>::reference>::value>::type* = nullptr>
  GCH_CPP20_CONSTEXPR void overwrite_existing_elements_with_allocator(
      ForwardIt first, ForwardIt last, alloc_interface& alloc) noexcept {
    destroy_range(begin_ptr(), end_ptr());
  ...
  template <typename ForwardIt,
            typename std::enable_if<!is_nothrow_emplace_insertable<
                typename std::iterator_traits<ForwardIt>::reference>::value>::type* = nullptr>
  GCH_CPP20_CONSTEXPR void overwrite_existing_elements_with_allocator(ForwardIt first,
                                                                      ForwardIt last,
  ...
  GCH_CPP20_CONSTEXPR void overwrite_existing_elements(small_vector_base<Allocator, N>&& other) {
    return overwrite_existing_elements(std::make_move_iterator(other.begin_ptr()),
                                       std::make_move_iterator(other.end_ptr()), other.get_size());
  }
  ...
  GCH_CPP20_CONSTEXPR void overwrite_existing_elements(small_vector_base<Allocator, N>&& other) {
    ...
    overwrite_existing_elements_with_allocator(std::make_move_iterator(other.begin_ptr()),
                                               std::make_move_iterator(other.end_ptr()), other);
  }

  template <typename ForwardIt>
  GCH_CPP20_CONSTEXPR small_vector_base& assign_with_range_with_allocator(ForwardIt first,
                                                                          ForwardIt last,
                                                                          alloc_interface& alloc) {
    ...
    if (InlineCapacity < count) {
      ...
      GCH_TRY { alloc.uninitialized_copy(first, last, new_data_ptr); }
      GCH_CATCH(...) {
...
#ifdef GCH_LIB_IS_CONSTANT_EVALUATED
      if (std::is_constant_evaluated()) new_data_ptr = alloc.allocate(InlineCapacity);
#endif
  ...
  template <unsigned N>
  GCH_CPP20_CONSTEXPR small_vector_base& copy_assign_equal_or_non_propagated_allocators(
      const small_vector_base<Allocator, N>& other) {
  ...
  template <unsigned N>
  GCH_CPP20_CONSTEXPR small_vector_base& copy_assign_unequal_and_propagated_allocators(
      const small_vector_base<Allocator, N>& other) {
  ...
  template <unsigned N, typename A = alloc_ty,
            typename std::enable_if<
                ...
                allocators_always_equal<A>::value>::type* = nullptr>
  GCH_CPP20_CONSTEXPR small_vector_base& copy_assign(const small_vector_base<Allocator, N>& other) {
  ...
  template <unsigned N>
  GCH_CPP20_CONSTEXPR small_vector_base& move_assign_pointer(
      small_vector_base<Allocator, N>& other) noexcept {
  ...
  template <unsigned N>
  GCH_CPP20_CONSTEXPR small_vector_base& move_assign_equal_or_non_propagated_allocators(
      small_vector_base<Allocator, N>& other) {
    assign_with_range(std::make_move_iterator(other.begin_ptr()),
                      std::make_move_iterator(other.end_ptr()), std::random_access_iterator_tag{});
    alloc_interface::maybe_move(std::move(other));
    return *this;
  }

  template <unsigned N>
  GCH_CPP20_CONSTEXPR small_vector_base& move_assign_unequal_and_propagated_allocators(
      small_vector_base<Allocator, N>& other) {
    assign_with_range_with_allocator(std::make_move_iterator(other.begin_ptr()),
                                     std::make_move_iterator(other.end_ptr()), other);
    return *this;
  }

  template <unsigned N = InlineCapacity, typename std::enable_if<N == 0>::type* = nullptr>
  GCH_CPP20_CONSTEXPR small_vector_base& move_assign_equal_allocators(small_vector_base& other) {
    return move_assign_pointer(other);
  }

  template <unsigned N>
  GCH_CPP20_CONSTEXPR small_vector_base& move_assign_equal_allocators(
      small_vector_base<Allocator, N>& other) {
  ...
  template <unsigned N = InlineCapacity, typename A = alloc_ty,
            typename std::enable_if<
                N == 0 && std::allocator_traits<A>::propagate_on_container_move_assignment::value>::
                type* = nullptr>
  GCH_CPP20_CONSTEXPR small_vector_base& move_assign_unequal_allocators(small_vector_base& other) {
    return move_assign_pointer(other);
  }

  template <unsigned N, typename A = alloc_ty,
            typename std::enable_if<std::allocator_traits<
                A>::propagate_on_container_move_assignment::value>::type* = nullptr>
  GCH_CPP20_CONSTEXPR small_vector_base& move_assign_unequal_allocators(
      small_vector_base<Allocator, N>& other) {
    if (other.has_allocation() && InlineCapacity < other.get_capacity())
      return move_assign_pointer(other);
    else
  ...
  template <unsigned N, typename A = alloc_ty,
            typename std::enable_if<allocators_always_equal<A>::value>::type* = nullptr>
  GCH_CPP20_CONSTEXPR small_vector_base& move_assign(small_vector_base<Allocator, N>& other) {
  ...
  template <unsigned N, typename A = alloc_ty,
            typename std::enable_if<!allocators_always_equal<A>::value>::type* = nullptr>
  GCH_CPP20_CONSTEXPR small_vector_base& move_assign(small_vector_base<Allocator, N>& other) {
    if (allocator_ref() == other.allocator_ref()) return move_assign_equal_allocators(other);
 ...
 public:
  ...
  GCH_CPP20_CONSTEXPR small_vector_base(bypass_tag, const small_vector_base<Allocator, I>& other,
                                        const A& alloc)
      : alloc_interface(alloc) {
    if (InlineCapacity < other.get_size()) {
      set_data_ptr(unchecked_allocate(other.get_size(), other.allocation_end_ptr()));
  ...
  template <unsigned I, typename A = alloc_ty,
            typename std::enable_if<!allocators_always_equal<A>::value>::type* = nullptr>
  GCH_CPP20_CONSTEXPR small_vector_base(bypass_tag, small_vector_base<Allocator, I>&& other,
  ...
  small_vector_base(size_ty count, const alloc_ty& alloc) : alloc_interface(alloc) {
    ...
    GCH_TRY { uninitialized_value_construct(begin_ptr(), unchecked_next(begin_ptr(), count)); }
    GCH_CATCH(...) {
      if (has_allocation()) deallocate(data_ptr(), get_capacity());
      GCH_THROW;
    }
  ...
  small_vector_base(size_ty count, const value_ty& val, const alloc_ty& alloc)
    ...
    GCH_TRY { uninitialized_fill(begin_ptr(), unchecked_next(begin_ptr(), count), val); }
    GCH_CATCH(...) {
      if (has_allocation()) deallocate(data_ptr(), get_capacity());
      GCH_THROW;
    }
  ...
  template <typename InputIt>
#endif
  GCH_CPP20_CONSTEXPR small_vector_base(InputIt first, InputIt last, std::input_iterator_tag,
                                        const alloc_ty& alloc)
  ...
  template <typename ForwardIt>
#endif
  GCH_CPP20_CONSTEXPR small_vector_base(ForwardIt first, ForwardIt last, std::forward_iterator_tag,
                                        const alloc_ty& alloc)
      : alloc_interface(alloc) {
 ...
 protected:
  ...
  void assign_with_copies(size_ty count, const value_ty& val) {
    if (get_capacity() < count) {
      ...
      GCH_TRY { uninitialized_fill(new_begin, unchecked_next(new_begin, count), val); }
      GCH_CATCH(...) {
  ...
  GCH_CPP20_CONSTEXPR void assign_with_range(const ForwardIt first, const ForwardIt last,
...
#ifdef GCH_LIB_IS_CONSTANT_EVALUATED
      if (std::is_constant_evaluated()) new_begin = this->allocate(InlineCapacity);
#endif
  ...
  GCH_CPP20_CONSTEXPR ptr uninitialized_move(ptr first, ptr last, ptr d_first) noexcept(
      std::is_nothrow_move_constructible<value_ty>::value) {
    return uninitialized_copy(std::make_move_iterator(first), std::make_move_iterator(last),
                              d_first);
  }
  ...
  GCH_CPP20_CONSTEXPR ptr emplace_into_reallocation_end(Args&&... args) {
    ...
    GCH_TRY {
      ...
      GCH_TRY { uninitialized_move<strong_exception_policy>(begin_ptr(), end_ptr(), new_data_ptr); }
      GCH_CATCH(...) {
        destroy(emplace_pos);
        GCH_THROW;
      }
  ...
  GCH_CPP20_CONSTEXPR void resize_with(size_ty new_size, const ValueT&... val) {
    ...
    if (get_capacity() < new_size) {
      ...
      GCH_TRY {
        new_last = uninitialized_fill(new_last, unchecked_next(new_data_ptr, new_size), val...);
  ...
  void request_capacity(size_ty request) {
    ...
    GCH_TRY { uninitialized_move<strong_exception_policy>(begin_ptr(), end_ptr(), new_begin); }
    GCH_CATCH(...) {
      deallocate(new_begin, new_capacity);
      GCH_THROW;
    }
  ...
  template <unsigned N>
  GCH_CPP20_CONSTEXPR void swap_elements_equal_or_non_propagated_allocators(
      small_vector_base<Allocator, N>& other) {
  ...
  template <
      unsigned N, typename T = value_ty,
      typename std::enable_if<std::is_nothrow_move_constructible<T>::value &&
                              is_explicitly_nothrow_move_insertable<T>::value>::type* = nullptr>
  GCH_CPP20_CONSTEXPR void swap_elements_unequal_and_propagated_allocators(
      small_vector_base<Allocator, N>& r) noexcept {
    if (r.get_size() < get_size()) return r.swap_elements_unequal_and_propagated_allocators(*this);
  ...
  template <
      ...
      typename std::enable_if<!std::is_nothrow_move_constructible<T>::value ||
                              !is_explicitly_nothrow_move_insertable<T>::value>::type* = nullptr>
  GCH_CPP20_CONSTEXPR void swap_elements_unequal_and_propagated_allocators(
      small_vector_base<Allocator, N>& r) {
    if (r.get_size() < get_size()) return r.swap_elements_unequal_and_propagated_allocators(*this);
    ...
    GCH_TRY {
      while (!(l_ptr == l_begin)) {
        ...
        GCH_TRY {
          ...
          r.construct(l_ptr, std::move(*r_ptr));
          r.destroy(r_ptr);
          GCH_TRY { construct(r_ptr, tmp.release()); }
          GCH_CATCH(...) {
            r.destroy(l_ptr);
            r.decrease_size(1);
            GCH_THROW;
  ...
  template <unsigned N, typename A = alloc_ty,
            typename std::enable_if<allocators_always_equal<A>::value>::type* = nullptr>
  void swap_elements(small_vector_base<Allocator, N>& other) {
    swap_elements_equal_or_non_propagated_allocators(other);
  }

  template <
      unsigned N, typename A = alloc_ty,
      typename std::enable_if<
          !allocators_always_equal<A>::value &&
          !std::allocator_traits<Allocator>::propagate_on_container_swap::value>::type* = nullptr>
  void swap_elements(small_vector_base<Allocator, N>& other) {
  ...
  template <unsigned N>
  GCH_CPP20_CONSTEXPR void swap_unequal_and_non_propagated_allocators(
      small_vector_base<Allocator, N>& other) {
    assert(allocator_ref() != other.allocator_ref() &&
  ...
  template <unsigned N = InlineCapacity, typename std::enable_if<N == 0>::type* = nullptr>
  GCH_CPP20_CONSTEXPR void swap_equal_or_propagated_allocators(small_vector_base& other) {
    m_data.swap(other.m_data);
    alloc_interface::maybe_swap(other);
  }

  template <unsigned LessEqualI>
  GCH_CPP20_CONSTEXPR void swap_equal_or_propagated_allocators(
      small_vector_base<Allocator, LessEqualI>& other) {
    static_assert(LessEqualI <= InlineCapacity, "should not be instantiated");
    assert(allocators_always_equal<Allocator>::value || allocator_ref() == other.allocator_ref() ||
           std::allocator_traits<Allocator>::propagate_on_container_swap::value);
    ...
    if (has_allocation()) {
      ...
      if (LessEqualI < get_size()) {
        ...
        new_data_ptr = alloc_interface::allocate_with_hint(new_capacity, allocation_end_ptr());
        GCH_TRY { uninitialized_move(begin_ptr(), end_ptr(), new_data_ptr); }
        GCH_CATCH(...) {
          deallocate(new_data_ptr, new_capacity);
          GCH_THROW;
        }
      ...
      ptr new_data_ptr =
          alloc_interface::allocate_with_hint(new_capacity, other.allocation_end_ptr());

      GCH_TRY {
        const ptr new_end = uninitialized_move(begin_ptr(), end_ptr(), new_data_ptr);
        GCH_TRY { overwrite_existing_elements(std::move(other)); }
        GCH_CATCH(...) {
  ...
  template <unsigned GreaterI, typename A = alloc_ty,
            typename std::enable_if<(InlineCapacity < GreaterI)>::type* = nullptr>
  GCH_CPP20_CONSTEXPR void swap(small_vector_base<Allocator, GreaterI>& other) {
    return other.swap(*this);
  }

  template <unsigned LessEqualI, typename A = alloc_ty,
            typename std::enable_if<(LessEqualI <= InlineCapacity) &&
                                    (std::allocator_traits<A>::propagate_on_container_swap::value ||
                                     allocators_always_equal<A>::value)>::type* = nullptr>
  GCH_CPP20_CONSTEXPR void swap(small_vector_base<Allocator, LessEqualI>& other) {
    return swap_equal_or_propagated_allocators(other);
  }

  template <
      unsigned LessEqualI, typename A = alloc_ty,
      typename std::enable_if<(LessEqualI <= InlineCapacity) &&
                              !(std::allocator_traits<A>::propagate_on_container_swap::value ||
                                allocators_always_equal<A>::value)>::type* = nullptr>
  GCH_CPP20_CONSTEXPR void swap(small_vector_base<Allocator, LessEqualI>& other) {
    if (allocator_ref() == other.allocator_ref()) return swap_equal_or_propagated_allocators(other);
  ...
  static GCH_CPP20_CONSTEXPR auto unmove_iterator(std::reverse_iterator<InputIt> it)
    ...
    return std::reverse_iterator<decltype(unmove_iterator(it.base()))>(unmove_iterator(it.base()));
  }
  ...
  template <typename RandomIt,
            typename std::enable_if<!is_memcpyable_iterator<RandomIt>::value &&
 ...
 public:
  ...
  GCH_NODISCARD GCH_CPP14_CONSTEXPR ptr data_ptr(void) noexcept { return m_data.data_ptr(); }

  GCH_NODISCARD constexpr cptr data_ptr(void) const noexcept { return m_data.data_ptr(); }

  GCH_NODISCARD constexpr size_ty get_capacity(void) const noexcept { return m_data.capacity(); }
...
#ifdef GCH_LIB_IS_CONSTANT_EVALUATED
    if (std::is_constant_evaluated()) return nullptr;
#endif
...
class small_vector : private detail::small_vector_base<Allocator, InlineCapacity> {
 ...
 private:
  ...
  struct EmplaceConstructible {
    static constexpr bool value =
        concepts::small_vector::EmplaceConstructible<value_type, small_vector, allocator_type,
                                                     Args...>;
  };
 ...
 public:
  ...
  GCH_CPP20_CONSTEXPR
  small_vector(void) noexcept(std::is_nothrow_default_constructible<allocator_type>::value)
#ifdef GCH_LIB_CONCEPTS
  ...
  GCH_CPP20_CONSTEXPR explicit small_vector(const allocator_type& alloc) noexcept : base(alloc) {}
  ...
  GCH_CPP20_CONSTEXPR explicit small_vector(size_type count, const allocator_type& alloc)
#ifdef GCH_LIB_CONCEPTS
  ...
  template <typename Generator,
            typename std::enable_if<
                ...
                !std::is_convertible<Generator, const allocator_type&>::value>::type* = nullptr>
#endif
  GCH_CPP20_CONSTEXPR small_vector(size_type count, Generator g, const allocator_type& alloc)
      : base(count, g, alloc) {
  }
  ...
  template <typename InputIt,
            typename std::enable_if<std::is_base_of<
                ...
                typename std::iterator_traits<InputIt>::iterator_category>::value>::type* = nullptr>
#endif
  GCH_CPP20_CONSTEXPR small_vector(InputIt first, InputIt last, const allocator_type& alloc)
...
#endif
  {
    if (&other != this) base::move_assign(other);
  }
...
#endif
      >
  GCH_CPP20_CONSTEXPR void swap(small_vector<T, I, Allocator>& other)
#ifdef GCH_LIB_CONCEPTS
    requires(MoveInsertable && MoveAssignable && Swappable) ||
            ((std::is_same<std::allocator<value_type>, Allocator>::value ||
              std::allocator_traits<Allocator>::propagate_on_container_swap::value
  ...
  constexpr const_iterator begin(void) const noexcept { return const_iterator{base::begin_ptr()}; }
  ...
  constexpr const_iterator end(void) const noexcept { return const_iterator{base::end_ptr()}; }
  ...
  constexpr size_type size(void) const noexcept { return static_cast<size_type>(base::get_size()); }

  GCH_NODISCARD constexpr bool empty(void) const noexcept { return size() == 0; }

  GCH_CPP14_CONSTEXPR
  size_type max_size(void) const noexcept { return static_cast<size_type>(base::get_max_size()); }

  constexpr size_type capacity(void) const noexcept {
    return static_cast<size_type>(base::get_capacity());
  }

  constexpr allocator_type get_allocator(void) const noexcept { return base::copy_allocator(); }
  ...
  GCH_NODISCARD constexpr bool inlined(void) const noexcept { return !base::has_allocation(); }
...
inline GCH_CPP20_CONSTEXPR void swap(
  ...
  requires concepts::MoveInsertable<T, small_vector<T, InlineCapacity, Allocator>, Allocator> &&
           concepts::MoveAssignable<T> && concepts::Swappable<T>
#endif
{
  lhs.swap(rhs);
}
...
template <typename T, unsigned InlineCapacityLHS, unsigned InlineCapacityRHS, typename Allocator
          ...
          >
inline GCH_CPP20_CONSTEXPR void swap(
    small_vector<T, InlineCapacityLHS, Allocator>& lhs,
    small_vector<T, InlineCapacityRHS, Allocator>& rhs) noexcept(noexcept(lhs.swap(rhs)))
#ifdef GCH_LIB_CONCEPTS
...
constexpr typename std::common_type<
    std::ptrdiff_t, typename std::make_signed<
